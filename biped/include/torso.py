...
def torso(goal_pos,time_limit):
 ...
 #   if ( data_check(goal_pos,time_limit) == 0):
        #end if value is not proper ie goal pos outof bound ot time limit too small
 #check for dataserver else popup a message
    ...
    while (rospy.is_shutdown() == 0 and time_count <= time_limit and start_pos[0]!=goal_pos[0] and start_pos[1]!=goal_pos[1] and start_pos[2]!=goal_pos[2] and start_pos[3]!=goal_pos[3]and start_pos[4]!=goal_pos[4]):
        global curr_pos; 
...
def data_check(goal_pos,time_limit):
    if (time_limit < 0.5):
        return 0;
